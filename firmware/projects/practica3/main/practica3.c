...
#define TIME_PERIOD1 250000 // 4 veces por seg -> cada 0.25 seg
...
uint16_t ConvertirVoltajeAPresion(presion_mV)
{
	float presion_bar = presion_mV / SENSIBILIDAD;
	return presion_bar;
}
...
void Task_controlarVolAguaTanques(void *pvParameter)
{
	...
	while (1)
	{
       ...
       if(tecla4)
	   {
		...
		UartSendString(UART_PC, UartItoa(litros_1, 10));
		UartSendString(UART_PC, "litros\r\n");

		UartSendString(UART_PC, "Tanque de agua sucia\r\n");
		UartSendString(UART_PC, UartItoa(litros_2, 10));
		UartSendString(UART_PC, "litros\r\n");
	}
	}
}
...
void Task_duchar(void *pvParameter)
{
	...
	while (1)
	{
		...
		presion_mbar = ConvertirVoltajeAPresion(presion_mV);
...
void app_main(void)
{
	...
	SwitchActivInt(SWITCH_4, &FunctionOnOff, NULL);
